...
# Task: Create Comprehensive Robot Vacuum Recommendation Report for Chilean Market (Premium Analysis)

## Objective
Analyze the robot vacuum inventory and source documents to create a tier-based recommendation report for the Chilean market, specifically tailored to vinyl floor cleaning. This analysis removes price constraints to evaluate premium models and determine if their advanced features justify the higher cost, especially considering upcoming Black Friday discounts.
...
3. **Source Documents**: `source_docs/` directory - Contains detailed evaluations and comparisons
4. **Web Sources**: `./analytics/inventory.md` - Contains web sources for verification and supplementation
5. **Previous Budget Analysis**: `analytics/recommendation.md` - Reference the winners from the ≤$700,000 CLP scenario for comparison

## User Requirements

### Critical Requirements (Must Have)
- **Price Range**: **NO BUDGET LIMIT** - Evaluate all models including premium options above $700,000 CLP
- **Floor Type**: Must be safe and effective for vinyl floors (primary requirement)
- **Coverage Area**: 80m² apartment
- **Battery Life**: Minimum 90 minutes continuous operation to cover entire area
- **Mopping System**: Reliable mopping with adjustable water flow and washable mop pads that won't damage vinyl
...
### Secondary Requirements
- **Availability**: Both online and in-store options in Chile (retailers like Falabella, Paris, Ripley, MercadoLibre)
- **Value Assessment**: Analyze if premium features justify higher prices, especially with potential Black Friday discounts
- **Use Case**: Daily cleaning & mopping with minimal intervention + occasional deep cleaning
- **Pet Hair**: Not a priority (no pets in household)
...
2. **Mopping Performance**: Quality of mopping system (rotating mops > vibrating > static)
3. **Automation Level**: Auto-emptying, self-cleaning mops, smart navigation
4. **Feature Completeness**: Advanced features that differentiate premium models
5. **Availability in Chile**: Local support, warranty, ease of purchase
6. **User Reviews**: Real-world feedback from Reddit, forums, and review sites
7. **Battery Life**: Ability to complete 80m² in one session
8. **Smart Home Compatibility**: Verified Alexa/Google/Home Assistant integration, or at least non-official support (like IFTTT, or Home Assistant community plugins)
...
### Evaluation Methodology
1. **Include ALL models** from inventory available in Chile (no price filtering)
2. **Prioritize** models with:
   - Confirmed vinyl floor compatibility
   - ≥90 minutes battery life
   - Auto-emptying station (preferred)
...
4. **Include** for each model:
   ...
   - User feedback summary with citations
   - Specific vinyl floor performance notes
   - **Premium feature analysis**: What extra features justify higher price
   - **Black Friday potential**: Estimated discount value and final cost-benefit

### Premium vs Budget Comparison
5. **Create dedicated comparison section**:
   - Compare top premium models (>$700,000 CLP) against previous budget winners
   - Analyze incremental features and their practical value
   - Calculate "premium tax" - extra cost per additional feature
   - Project Black Friday pricing scenarios (typical 15-30% discounts)
   - Determine if premium models become "best value" at discounted prices

## Deliverable Specifications

**Output File**: `./analytics/recommendation_premium.md`

**Required Sections**:
1. **Executive Summary**: Top 3 recommendations across ALL price ranges with brief rationale
2. **Tier Table**: Complete tier classification (S/A/B/C/D) with all qualifying models (no price limit)
3. **Premium Model Deep Dive**: Detailed analysis of models >$700,000 CLP:
   - What premium features they offer
   - Real-world performance advantages
   - Whether features justify the price premium
4. **Budget vs Premium Comparison**: 
   - Side-by-side comparison of previous budget winners vs premium alternatives
   - Feature gap analysis
   - Price-to-performance ratio
   - Black Friday discount scenarios and value proposition shifts
5. **Detailed Model Analysis**: For each tier, provide:
   - Model specifications relevant to user requirements
   - Vinyl floor compatibility assessment
   - Mopping system evaluation
   - Automation features
   ...
   - User feedback summary with source citations
   - Pros/cons specific to user's use case
6. **Black Friday Strategy**: 
   - Which premium models become "best buy" at 15%, 20%, 25%, 30% discounts
   - Price alerts and monitoring recommendations
   - Historical Black Friday pricing data (if available)
7. **Purchase Recommendations**: Where to buy in Chile (online/in-store) with current pricing
8. **Final Recommendation**: 
   - Best choice at current prices
   - Best choice if Black Friday discounts materialize
...
- Include user review sources (Reddit threads, forums)
- Cite Chilean retailer pricing where available
- Reference previous budget analysis recommendations
